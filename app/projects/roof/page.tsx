...
import { ArrowLeft, ArrowRight, Clock3, Home, Ruler, Tag } from "lucide-react";
...
import { estimateRoofLaborHours, generateRoofCatalog, type RoofTier } from "@/lib/cart-engine/roof-catalog";
import { EASE, fadeUp } from "@/lib/motion";
import { startRoofProject } from "@/lib/storage";
import { formatSEK, todayKey } from "@/lib/utils";

const BUILDING_STORES = storesForDomain("building");
const TIER_OPTIONS = [
  { value: "budget", label: "Plåt" },
  { value: "premium", label: "Tegel" },
] as const;
...
/**
 * "Tak" — fjärde kalkylatorn i samma mönster: en yta plus
 * följdfrågor in, en fullt kvantifierad materiallista ut. Delar
 * "building"-domänen med Bygga altan, Innervägg, Golv och Målning.
 */
export default function RoofProjectPage() {
  const router = useRouter();
  const [areaM2, setAreaM2] = useState(80);
  const [rannor, setRannor] = useState(true);
  const [tier, setTier] = useState<RoofTier>("budget");
  const [planGenerated, setPlanGenerated] = useState(false);
  const [scannedCount, setScannedCount] = useState(0);

  const opts = useMemo(() => ({ areaM2, rannor, tier }), [areaM2, rannor, tier]);

  const materials = useMemo(() => (planGenerated ? generateRoofCatalog(opts) : []), [planGenerated, opts]);
  const laborHours = useMemo(() => estimateRoofLaborHours(opts), [opts]);
    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden px-6 py-10 sm:py-14">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center">
        <motion.div {...fadeUp(0)} className="text-center">
          ...
          <h1 className="text-balance text-2xl font-bold tracking-tight sm:text-4xl">Tak</h1>
          <p className="mt-3 text-ink-secondary">
            {planGenerated
              ? "AI har brutit ner projektet i vad du faktiskt behöver."
              : "Ange takytan och svara på några frågor. AI räknar ut exakt vad du behöver."}
          </p>
        </motion.div>
        ...
        {!planGenerated ? (
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              ...
              <div>
                <FieldLabel>Behövs hängrännor?</FieldLabel>
                <YesNoToggle value={rannor} onChange={setRannor} />
              </div>

              <div>
                <FieldLabel>Plåt eller tegel?</FieldLabel>
                <SingleChipGroup options={TIER_OPTIONS} value={tier} onChange={(v) => setTier(v as RoofTier)} />
              </div>
          ...
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              <div className="flex items-end justify-between gap-4">
                <CardTitle>Tak {areaM2} m²</CardTitle>
                {!scanning && (
