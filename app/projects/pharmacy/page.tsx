...
import { ArrowLeft, ArrowRight, Pill, Tag } from "lucide-react";
...
import { APOTEK_ITEM_OPTIONS, generateApotekCatalog } from "@/lib/cart-engine/apotek-catalog";
import { EASE, fadeUp } from "@/lib/motion";
import { startApotekProject } from "@/lib/storage";
import { formatSEK, todayKey } from "@/lib/utils";

const APOTEK_STORES = storesForDomain("pharmacy");
const FULL_CATALOG = generateApotekCatalog();
...
export default function PharmacyProjectPage() {
  const router = useRouter();
  const [selectedIds, setSelectedIds] = useState<string[]>(APOTEK_ITEM_OPTIONS.slice(0, 3).map((o) => o.value));
  ...
  const scanning = planGenerated && scannedCount < APOTEK_STORES.length;
  ...
  useEffect(() => {
    ...
    setScannedCount(0);
    const timers = APOTEK_STORES.map((_, i) =>
  ...
  const handleContinue = () => {
    startApotekProject(selectedIds);
    router.push("/cart");
  };
    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden px-6 py-10 sm:py-14">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center">
        <motion.div {...fadeUp(0)} className="text-center">
          ...
          <h1 className="text-balance text-2xl font-bold tracking-tight sm:text-4xl">Apotek</h1>
          <p className="mt-3 text-ink-secondary">
            {planGenerated
              ? "AI har brutit ner projektet i vad du faktiskt behöver."
              : "Vad behöver du? AI jämför fyra apotek åt dig."}
          </p>
        </motion.div>
        ...
        {!planGenerated ? (
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              <div>
                <FieldLabel>Vad ska du handla?</FieldLabel>
                <MultiChipGroup options={APOTEK_ITEM_OPTIONS} values={selectedIds} onChange={setSelectedIds} />
              </div>
