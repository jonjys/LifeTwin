...
import { ArrowLeft, ArrowRight, Clock3, Paintbrush, Ruler, Tag } from "lucide-react";
...
import { estimatePaintLaborHours, generatePaintCatalog, type PaintTier } from "@/lib/cart-engine/paint-catalog";
import { EASE, fadeUp } from "@/lib/motion";
import { startPaintProject } from "@/lib/storage";
...
/**
 * "Målning" — the third calculator to prove Innervägg's pattern
 * generalizes: one dimension (yta) plus follow-up questions feed
 * lib/cart-engine/paint-catalog.ts, sharing the "building" domain with
 * Bygga altan, Innervägg och Golv.
 */
export default function PaintProjectPage() {
  const router = useRouter();
  const [areaM2, setAreaM2] = useState(25);
  const [tak, setTak] = useState(false);
  const [verktyg, setVerktyg] = useState(true);
  const [tier, setTier] = useState<PaintTier>("budget");
  const [planGenerated, setPlanGenerated] = useState(false);
  const [scannedCount, setScannedCount] = useState(0);

  const opts = useMemo(() => ({ areaM2, tak, verktyg, tier }), [areaM2, tak, verktyg, tier]);

  const materials = useMemo(() => (planGenerated ? generatePaintCatalog(opts) : []), [planGenerated, opts]);
  const laborHours = useMemo(() => estimatePaintLaborHours(opts), [opts]);
    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden px-6 py-10 sm:py-14">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center">
        <motion.div {...fadeUp(0)} className="text-center">
          ...
          <h1 className="text-balance text-2xl font-bold tracking-tight sm:text-4xl">Målning</h1>
          <p className="mt-3 text-ink-secondary">
            {planGenerated
              ? "AI har brutit ner projektet i vad du faktiskt behöver."
              : "Ange ytan och svara på några frågor. AI räknar ut exakt vad du behöver."}
          </p>
        </motion.div>
          ...
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  <FieldLabel>Ska taket också målas?</FieldLabel>
                  <YesNoToggle value={tak} onChange={setTak} />
                </div>
                <div>
              ...
              <div>
                ...
                <SingleChipGroup options={TIER_OPTIONS} value={tier} onChange={(v) => setTier(v as PaintTier)} />
              </div>
          ...
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              <div className="flex items-end justify-between gap-4">
                <CardTitle>Målning {areaM2} m²</CardTitle>
                {!scanning && (
