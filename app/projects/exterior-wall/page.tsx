...
import {
  ...
  type ExteriorWallTier,
} from "@/lib/cart-engine/exterior-wall-catalog";
import { EASE, fadeUp } from "@/lib/motion";
import { startExteriorWallProject } from "@/lib/storage";
...
/**
 * "Yttervägg" — mirrors Innervägg (bredd × höjd, samma "building"-domän)
 * men med väderskyddande material: vindskydd, fasadpanel, isolering och
 * fasadfärg som tillval.
 */
export default function ExteriorWallProjectPage() {
  ...
  const [tier, setTier] = useState<ExteriorWallTier>("budget");
  const [planGenerated, setPlanGenerated] = useState(false);
  const [scannedCount, setScannedCount] = useState(0);

  const opts = useMemo(
    () => ({ widthM, heightM, isolera, malas, tier }),
    [widthM, heightM, isolera, malas, tier]
  );

  const materials = useMemo(
    () => (planGenerated ? generateExteriorWallCatalog(opts) : []),
    [planGenerated, opts]
  );
  const laborHours = useMemo(() => estimateExteriorWallLaborHours(opts), [opts]);
    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden px-6 py-10 sm:py-14">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center">
        <motion.div {...fadeUp(0)} className="text-center">
          ...
          <h1 className="text-balance text-2xl font-bold tracking-tight sm:text-4xl">Yttervägg</h1>
            ...
            <Card className="flex flex-col gap-5">
              ...
              <div>
                ...
                <SingleChipGroup
                  ...
                  value={tier}
                  onChange={(v) => setTier(v as ExteriorWallTier)}
                />
              </div>
