...
import { ArrowLeft, ArrowRight, Ruler, SquareParking, Tag } from "lucide-react";
...
import { startParkingProject } from "@/lib/storage";
import { formatSEK, todayKey } from "@/lib/utils";

const BUILDING_STORES = storesForDomain("building");
const TIER_OPTIONS = [
  { value: "budget", label: "Grus" },
  { value: "premium", label: "Asfalt" },
] as const;
...
/**
 * "Parkering" — samma mönster som Isolering: en yta plus följdfrågor in,
 * en fullt kvantifierad materiallista ut. Delar "building"-domänen med
 * resten av Bygg.
 */
export default function ParkingProjectPage() {
  const router = useRouter();
  const [areaM2, setAreaM2] = useState(20);
  const [kantsten, setKantsten] = useState(false);
  const [tier, setTier] = useState<ParkingTier>("budget");
  const [planGenerated, setPlanGenerated] = useState(false);
  const [scannedCount, setScannedCount] = useState(0);

  const opts = useMemo(() => ({ areaM2, kantsten, tier }), [areaM2, kantsten, tier]);

  const materials = useMemo(
    () => (planGenerated ? generateParkingCatalog(opts) : []),
    [planGenerated, opts]
  );
  const laborHours = useMemo(() => estimateParkingLaborHours(opts), [opts]);
    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden px-6 py-10 sm:py-14">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center">
        <motion.div {...fadeUp(0)} className="text-center">
          ...
          <h1 className="text-balance text-2xl font-bold tracking-tight sm:text-4xl">Parkering</h1>
        ...
        {!planGenerated ? (
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              ...
              <div>
                <FieldLabel>Behövs kantsten?</FieldLabel>
                <YesNoToggle value={kantsten} onChange={setKantsten} />
              </div>

              <div>
                <FieldLabel>Grus eller asfalt?</FieldLabel>
                <SingleChipGroup options={TIER_OPTIONS} value={tier} onChange={(v) => setTier(v as ParkingTier)} />
              </div>
          ...
          <motion.div {...fadeUp(0.1)} className="mt-6">
            <Card className="flex flex-col gap-5">
              <div className="flex items-end justify-between gap-4">
                <CardTitle>Parkering {areaM2} m²</CardTitle>
                {!scanning && (
